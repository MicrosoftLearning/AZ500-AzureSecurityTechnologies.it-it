...
2. Nel pannello **Hub del contenuto di Microsoft Sentinel \|** esaminare l'elenco dei contenuti disponibili.
...
5. Nel pannello **Connettori dati di Microsoft Sentinel \|** fare clic su **Aggiorna** ed esaminare l'elenco dei connettori disponibili. Selezionare la voce che rappresenta il connettore **attività di Azure** (nascondere la barra dei menu a sinistra usando \<<, se necessario), esaminare la descrizione e lo stato all'estrema destra e quindi fare clic su **Apri la pagina connettore**.
...
4. Nella scheda **Generale** della **procedura guidata delle regole di Analisi - Creare un nuovo pannello Regole pianificate** accettare le impostazioni predefinite e fare clic su **Avanti: Imposta logica delle regole >**.

5. Nella scheda **Imposta logica regola** della **procedura guidata analisi - Creare un nuovo pannello Regole pianificate** accettare le impostazioni predefinite e fare clic su **Avanti: Impostazioni evento imprevisto (anteprima) >**.

6. Nella scheda **Impostazioni evento imprevisto** della **procedura guidata delle regole di analisi - Creare un nuovo** pannello Regole pianificate accettare le impostazioni predefinite e fare clic su **Avanti: Risposta automatizzata >**. 
...
7. Nella scheda **Risposta automatica** della **procedura guidata di Analisi - Creare un nuovo** pannello Regole pianificate accettare le impostazioni predefinite e fare clic su **Avanti: Rivedi e crea >**. 

8. Nella scheda **Rivedi e crea** della **procedura guidata delle regole di Analisi - Crea una nuova regola pianificata** fare clic su **Salva**.
...
10. Nel pannello **Change-Incident-Severity** fare clic su **Modifica**.
    ...
    >**Nota**: nel pannello App per la **logica Designer** ognuno dei quattro visualizza un avviso. con cui si indica che ciascuna di esse deve essere controllata e configurata.

11. Nel pannello App per la **logica Designer** fare clic **sul primo passaggio**.
...
14. Fare clic **sul secondo passaggio** e, nell'elenco di s, selezionare la seconda voce, che rappresenta l'oggetto creato nel passaggio precedente.
...
4. Nella scheda **Generale** della **procedura guidata regole di Analisi - Creare un nuovo pannello Regole pianificate** specificare le impostazioni seguenti (lasciare altri con i relativi valori predefiniti):
...
6. Nella scheda **Imposta logica regola** della **procedura guidata analisi - Creare un nuovo pannello Regole pianificate** , nella casella di testo **Query regola** incollare la query di regola seguente. 
    ...
    >**Nota**: questa regola identifica la rimozione dei criteri di accesso alle macchine virtuali just-in-time.
...
7. Nella scheda **Imposta logica regola** della **procedura guidata Analisi - Creare un nuovo** pannello Regole **pianificate** , nella sezione Pianificazione query impostare la **query Esegui ogni** **5 minuti**.

8. Nella scheda **Imposta logica regola** della **procedura guidata Analisi - Creare un nuovo pannello Regole pianificate** accettare i valori predefiniti delle impostazioni rimanenti e fare clic su **Avanti: Impostazioni evento imprevisto >**.

9. Nella scheda **Impostazioni evento imprevisto** della **procedura guidata analisi - Creare un nuovo pannello Regole pianificate** accettare le impostazioni predefinite e fare clic su **Avanti: Risposta automatizzata >**. 

10. Nella scheda **Risposta automatizzata** della **procedura guidata analisi - Creare un nuovo** pannello Regole pianificate, in **Regole di automazione** fare clic su **+ Aggiungi nuovo**.

11. Nella finestra **Crea nuova regola di automazione** immettere **Esegui Change-Severity Playbook** per il **nome della regola di automazione**; nel campo **Trigger** fare clic sul menu a discesa e selezionare **Quando viene creato l'avviso**.

12. Nella finestra **Crea nuova regola di automazione** , in **Azioni** leggere la nota e quindi fare clic su **Gestisci autorizzazioni del playbook**. Nella finestra **Gestisci autorizzazioni** selezionare la casella di controllo accanto al **gruppo di risorse** creato in precedenza AZ500LAB1314151 e quindi fare clic su **Applica**.

13.  Nella finestra **Crea nuova regola di automazione** fare clic **** sul secondo menu a discesa e selezionare l'app per la logica **Change-Incident-Graviy**. Nella finestra **Crea nuova regola di automazione** fare clic su **Applica**.

14. Nella scheda **Risposta automatica** della **procedura guidata analisi - Creare un nuovo** pannello Regole pianificate fare clic su **Avanti: Rivedere e creare >e ** fare clic su **Salva**
...
2. Nel pannello **panoramica Microsoft Defender per cloud \|** fare clic su **Protezione dei carichi di lavoro** in **Sicurezza cloud** nella navigazione a sinistra.

3. Nel pannello **Microsoft Defender per la protezione dei carichi di lavoro cloud \|** scorrere verso il basso e fare clic sul riquadro **Di accesso alle macchine virtuali just-in-time** in **Protezione avanzata**.

4. Nel pannello **Accesso alle macchine virtuali just-in-time** fare clic **sul pulsante** **Rimuovi** e quindi fare clic su **Sì**.****
...
6. Passare al pannello **Log attività** e verificare che sia presente l'opzione **Delete JIT Network Access Policies** (Elimina criteri di accesso alla rete JIT). 
    ...
    >**Nota**: è possibile che vengano visualizzati alcuni minuti. **Aggiornare** la pagina se non viene visualizzata.

7. Nel portale di Azure tornare al pannello **Microsoft Sentinel \| Panoramica**.

8. Nel pannello **Panoramica di Microsoft Sentinel \|** esaminare il dashboard e verificare che venga visualizzato un evento imprevisto corrispondente all'eliminazione dei criteri di accesso alle macchine virtuali just-in-time.

    >**Nota**: la visualizzazione degli avvisi nel pannello **Microsoft Sentinel \| Panoramica** può richiedere fino a 5 minuti. Se non viene visualizzato un avviso a questo punto, eseguire la regola di query a cui si fa riferimento nell'attività precedente per verificare che l'attività di eliminazione dei criteri di accesso just-in-time sia stata propagata all'area di lavoro Log Analytics associata all'istanza di Microsoft Sentinel. In caso contrario, ricreare i criteri di accesso alle macchine virtuali just-in-time ed eliminarli di nuovo.
...
> Risultati: è stata creata un'area di lavoro di Microsoft Sentinel, connessa ai log attività di Azure, è stato creato un playbook e avvisi personalizzati attivati in risposta alla rimozione dei criteri di accesso alle macchine virtuali just-in-time e si è verificato che la configurazione sia valida.
